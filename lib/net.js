...
function setImplmentationMethods (self) {
  ...
  if (self.secure) {
    ...
    self._secureCycle = function() {
      ...
      while (self._encIn_pending.length > 0) {
        tmp = self._encIn_pending.shift();

        try {
         rv = self.secureStream.encIn(tmp, 0, tmp.length);
        } catch (e) {
          return self.destroy(e);
        }

        if (rv === 0) {
          self._encIn_pending.unshift(tmp);
          break;
        }
      ...
      while (self.secureEstablished && self._clearIn_pending.length > 0) {
        tmp = self._clearIn_pending.shift();
        try {
          rv = self.secureStream.clearIn(tmp, 0, tmp.length);
        } catch (e) {
          return self.destroy(e);
        }

        if (rv === 0) {
          self._clearIn_pending.unshift(tmp);
          break;
        }
      ...
      do {
        ...
        do {
          try {
            chunkBytes = self.secureStream.encOut(pool,
                                                  pool.used + bytesRead,
                                                  pool.length - pool.used - bytesRead);
          } catch (e) {
            return self.destroy(e);
        ...
        if (bytesRead > 0) {
          chunk = pool.slice(0, bytesRead);
          self._write_state = self._socketWriteOut(chunk);
        }
    ...
    self._writeOut = function (data, encoding, fd) {
      ...
      self._clearIn_pending.push(buffer.slice(off, off+len));
      self._secureCycle();

      /* Set to the value from the actual socket write call inside _secureCycle. */
...
function initStream (self) {
  ...
  self._readWatcher.callback = function () {
    ...
    if (bytesRead === 0) {
      ...
      if (self.secure) {
        self._encIn_pending.push(pool.slice(start, end));
        self._secureCycle();
      } else {
        if (self._decoder) {
          // emit String
      ...
      if (bytesRead !== -1) {
        self.destroy(new Error('unknown read error: '+ bytesRead));
      }
      return false;
    }
  };
...
Stream.prototype._startSecureHandshake = function () {
  this.secureStream.start();
  if (this.server) {
...
Stream.prototype._socketWriteOut = Stream.prototype._writeOut;
...
Stream.prototype.flush = function () {
  ...
  while (this._writeQueue && this._writeQueue.length) {
    ...
    var flushed = this._socketWriteOut(data,encoding,fd);
