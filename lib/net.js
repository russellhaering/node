  ...
  if (self.secure) {
    ...
    self._secureCycle = function() {
      ...
      while (self._encIn_pending.length > 0) {
        ...
        try {
         rv = self.secureStream.encIn(tmp[0], tmp[1], tmp[2]);
        } catch (e) {
      ...
      while (self.secureEstablished && self._clearIn_pending.length > 0) {
        ...
        try {
          rv = self.secureStream.clearIn(tmp[0], tmp[1], tmp[2]);
        } catch (e) {
          return self.destroy(e);
        }
    ...
    self._writeOut = function (data, encoding, fd) {
      ...
      self._clearIn_pending.push([buffer, off, len]);
      self._secureCycle();
...
function initStream (self) {
  ...
  self._readWatcher.callback = function () {
    ...
    if (bytesRead === 0) {
      ...
      if (self.secure) {
        self._encIn_pending.push([pool, start, bytesRead]);
        self._secureCycle();
      } else {
        if (self._decoder) {
          // emit String
