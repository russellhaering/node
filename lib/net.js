...
function _doFlush () {
  ...
  if (socket.flush()) {
    if (socket._events && socket._events['drain']) {
      socket.emit("drain");
    }
    if (socket.ondrain) {
...
function setImplmentationMethods (self) {
  ...
  if (self.secure) {
    var oldWriteOut = self._writeOut;
    self._write_state =  true;

    /* Attempt to cycle OpenSSLs buffers in various directions */
    self._secureCycle = function() {
      debug('start secure cycle');
      var bytesRead;
      var bytesWritten;
      var chunkBytes;
      var chunk = null;
      ...
      do {
        ...
        pool.used = 0;
        do {
          chunkBytes = self.secureStream.clearOut(pool, pool.used + bytesRead, pool.length - pool.used - bytesRead);
        ...
        if (bytesRead > 0) {
          ...
          if (self.ondata) {
            self.ondata(pool, 0, chunk.length);
          }
        }
      } while (bytesRead > 0);
      ...
      do {
        ...
        do {
          chunkBytes = self.secureStream.encOut(pool, pool.used + bytesRead, pool.length - pool.used - bytesRead);
          bytesRead += chunkBytes;
        ...
        if (bytesRead > 0) {
          chunk = pool.slice(0, bytesRead);
          debug('Read ' + bytesRead + ' bytes to write enc');
          self._write_state = oldWriteOut.call(self, chunk);
        }
      } while (bytesRead > 0 && self._write_state === true);
    ...
    self._writeOut = function (data, encoding, fd) {
      debug('writeOut entry');
      var buffer = null;
      var off = 0;
      var len = 0;
      ...
      self.secureStream.clearIn(buffer, off, len);
    ...
    self._shutdownImpl = function () {
      debug('shutdown entry');
      self.secureStream.shutdown();

      self._secureCycle();

      oldShutdown.call(self);
    };
  }
};

function initStream (self) {
  debug('initStream entry');
  ...
  self._readWatcher.callback = function () {
    ...
    if (bytesRead === 0) {
      ...
      if (self.secure) {
        self.secureStream.encIn(pool, start, bytesRead);
...
exports.createSSLConnection = function (port, host, creds) {
  var s = new Stream();
  s.connect(port, host);
  return new SSLStream(s, creds, false);
};
...
function doConnect (socket, port, host) {
  ...
  socket._writeWatcher.callback = function () {
    ...
    if (errno == 0) {
      ...
      debug('writable: '+ socket.writable);
...
function SSLStream(parent_stream, credentials, is_server)
{
  if (!(this instanceof SSLStream)) return new SSLStream(parent_stream, credentials, is_server);
  stream.Stream.call(this);
  ...
  this.writeable = true;
  this.readable = true;
  this.kBufferSize = 16 * 1024;
  this.secure = true;
  this.secureEstablished = false;
  this.server = is_server ? true : false;
  ...
  this.on('secure', function() {
    self._pending.forEach(function(v) {
      self.write(v);
    });
...
util.inherits(SSLStream, stream.Stream);
exports.SSLStream = SSLStream;

SSLStream.prototype._cycle = function()
{
  /* Attempt to cycle OpenSSLs buffers in all directions */
  var bytesRead = 0;
  var chunkBytes = 0;
  var chunk = null;
  var pool = new Buffer(this.kBufferSize);
  pool.used = 0;
  do {
    chunkBytes = this._ssl.clearOut(pool, pool.used + bytesRead, pool.length - pool.used - bytesRead);
    bytesRead += chunkBytes;
  } while ((chunkBytes > 0) && (pool.used + bytesRead < pool.length));
  ...
  pool = new Buffer(this.kBufferSize);
  pool.used = 0;
  bytesRead = 0;
  chunkBytes = 0;

  do {
    chunkBytes = this._ssl.encOut(pool, pool.used + bytesRead, pool.length - pool.used - bytesRead);
    bytesRead += chunkBytes;
  } while ((chunkBytes > 0) && (pool.used + bytesRead < pool.length));
  ...
  if (bytesRead > 0) {
    debug('Read ' + bytesRead + ' bytes to write enc');
    chunk = pool.slice(0, bytesRead);
    this.write_state = this._stream.write(chunk);
    debug('write state: '+ this.write_state)
  }

  if (!this.secureEstablished && this._ssl.isInitFinished()) {
...
SSLStream.prototype.pause = function()
{
  this._stream.pause();
};

SSLStream.prototype.resume = function()
{
  this._stream.resume();
};
...
/* writable methods */
SSLStream.prototype.destroy = function()
{
...
SSLStream.prototype.end = function(data, encoding)
{
  if (data) {
...
SSLStream.prototype.write = function (data, encoding, fd)
{
  this._pending.push(data);

  while (this._pending.length !== 0) {
    var buf = this._pending.shift();
    debug('Writing '+ buf.length +' bytes cleartext');
    var bytesWritten = this._ssl.clearIn(data, 0, data.length);
    debug('bytesWritten: '+ bytesWritten);
    debug('data.length: '+ buf.length);
    if (bytesWritten === 0) {
      this._pending.unshift(buf);
      break;
    }
  }
...
SSLStream.prototype._read = function (data)
{
  var bytesWritten = this._ssl.encIn(data, 0, data.length);
  debug('Read '+ bytesWritten +' bytes encrypted');
  this._cycle();
  return bytesWritten;
};
