...
OutgoingMessage.prototype.end = function (data, encoding) {
  ...
  if (this._hot && 'string' === typeof data) {
    if (this.chunkedEncoding) {
      data = this._combined + Buffer.byteLength(data, encoding) + CRLF + data +
             '\r\n0\r\n' + this._trailer + '\r\n';
    } else {
      data = this._combined + data;
    }
    ...
    if (this.chunkedEncoding) {
      this._output = [this._combined + data.length + CRLF, data,
                      '\r\n0\r\n' + this._trailer + '\r\n'];
      this._outputEncoding = ['ascii', undefined, 'ascii'];
    } else {
      this._output = [this._combined, data];
      this._outputEncoding = ['ascii', undefined];
    }
...
OutgoingMessage.prototype._cycle = function() {
  ...
  if (!this._parent._stream.writable) {
