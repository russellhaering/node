...
OutgoingMessage.prototype.end = function (data, encoding) {
  // Hot
  if ('string' === typeof data && this._hot) {
    if (this.chunkedEncoding) {
      this._combined += Buffer.byteLength(data, encoding) + CRLF + data +
                        '\r\n0\r\n' + this._trailer + '\r\n';
      data = new Buffer(this._combined, encoding);
    } else {
      data = new Buffer(this._combined + data, encoding);
    }

    this._writeState = this._parent._childWrite(this, data);
  } else {
    if (data) {
