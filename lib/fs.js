...
fs.createBufferedWriteStream = function(path, options) {
  return new BufferedWriteStream(path, options);
};

var BufferedWriteStream = fs.BufferedWriteStream = function(path, options) {
  events.EventEmitter.call(this);
  ...
  this.encoding = 'binary';
  this.mode = 0666;
  this.maxWriteTime = 2500;
  this.maxWriteSize = 5 * 1048576;
  /* TODO: IOVEC_MAX is operating system specific, find a better way to set this*/
  this.maxWriteSegments = 1024;
...
sys.inherits(BufferedWriteStream, events.EventEmitter);
...
BufferedWriteStream.prototype._flushWrites = function() {
  var self = this;
  this.accepting_writes = false;
  if (this._writeTimer !== null) {
    clearTimeout(this._writeTimer);
    this._writeTimer = null;
  }
  
  //console.log("Flushing writes: qlength="+ this._writeQueue.length + " qsize="+ this._writeQueueSize);

  /* TODO: Add writev binding */
  for (var i = 0; i < this._writeQueue.length; i++) {
    var eq = this._writeQueue[i];
    this._queue.push([fs.write, eq[0], 0, eq[0].length, null, function() {
      self.flush();
      if (eq[1]) {
        eq[1]()
      }
...
BufferedWriteStream.prototype._startWriteTimer = function() {
  var self = this;
...
BufferedWriteStream.prototype._enqueueWrite = function(buf, cb) {
  this._writeQueue.push([buf, cb]);
  this._writeQueueSize += buf.length;

  if (this._writeQueueSize >= this.maxWriteSize) {
    this._flushWrites();
  }
  else if (this._writeQueue.length >= this.maxWriteSegments) {
    this._flushWrites();
  }
  else {
    if (this._writeTimer === null) {
...
BufferedWriteStream.prototype.write = function (data) {
  if (!this.writeable) {
    throw new Error('stream not writeable');
  }
  ...
  /* TODO: because buffer ownership rules are not clear, we are defaulting
   * to copying all data here, because a consumer might be using a ring buffer
   */
  /* TODO: even though we are copying here, we could copy into our own pool of pre-allocated
   * buffers, if figuring out ownership stays hard.
   */
  if (data instanceof Buffer) {
    var tbuf = new Buffer(data.length);
    data.copy(tbuf, 0, 0);
    this._enqueueWrite(tbuf, cb);
  } else {
    var encoding = 'utf8';
    if (typeof(arguments[1]) == 'string') encoding = arguments[1];
    var tbuf = new Buffer(data, encoding);
    this._enqueueWrite(tbuf, cb);
  }
  ...
  return this.accepting_writes;
};


BufferedWriteStream.prototype.end = function (cb) {
  this.accepting_writes = false;
  this.writeable = false;
  this._flushWrites();
  this._queue.push([fs.close, cb]);
...
BufferedWriteStream.prototype.destroy = function (cb) {
  this.accepting_writes = false;
  this.writeable = false;
  fs.close(self.fd, function(err) {
    if (err) {
