...
function ClientConnection (server, stream, secure, creds) {
  ...
  if (secure) {
    ...
    stream.on('data', function(data) {
      streamOnData.call(stream, data, 0, data.length)
    })
  } else {
    // Fast case.
...
var parserOnIncoming = function(request, shouldKeepAlive) {
  ...
  response.connection = self._stream;
...
var streamOnData = function (data, start, end) {
  ...
  if (ret instanceof Error) {
    ...
    var upgradeHead = data.slice(bytesParsed + 1, end);

    if (client._server.listeners("upgrade").length) {
      client._server.emit('upgrade', request, client._stream, upgradeHead);
    } else {
...
ClientConnection.prototype.pause = function() {
  this._stream.pause();
}

ClientConnection.prototype.resume = function() {
  this._stream.resume();
}
...
ClientConnection.prototype._childDone = function(response, error, isIncoming) {
  ...
  if (!isIncoming) {
    ...
    if ((!this._reading || response._last === true) &&
      ...
      this._stream._endflush(function() {
        self.destroy();
      });
