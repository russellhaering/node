...
function SecureFilter(source, target, credentials, is_server)
{
  ...
  this._source.on('data', function(data) {
    debug('clearIn data');
    this._clearIn_pending.push(data);
    this._cycle();
  });

  this._taget.on('data', function(data) {
    debug('encIn data');
...
SecureFilter.prototype.end = function(data, encoding)
{
  if (data) {
...
SecureFilter.prototype.write = function (chunk)
{
  return this._write_state;
};

SecureFilter.prototype._read = function (data)
{
  this._encIn_pending.push(data);
  this._cycle();
};
