...
function SecurePair(credentials, isServer) {
  ...
  this._cleartextWriteState = true;

  /* Acts as a r/w stream to the encrypted side of the stream. */
  this.encrypted = new stream.Stream();
  this.encrypted.readable = true;
  this.encrypted.writable = true;
  this._encryptedWriteState = true;

  this.cleartext.verifyPeer = function() {
    return self.verifyPeer();
  };

  this.cleartext.getPeerCertificate = function() {
    return self.getPeerCertificate();
  };

  this.cleartext.write = function(data, encoding) {
    debug('clearIn data');
  ...
  this.encrypted._endflush = function(func) {
    self.encrypted._peerStream._endflush(func);
  }
  ...
  this.encrypted.end = function(err) {
    ...
    if (self._done !== true) {
      try {
        ...
        /* This is less than ideal, but we wanted to try to be 'nice' */
      }
    }
    self._destroy(err);
  };
  ...
  this.cleartext.on('end', function(err) {
    debug('clearIn end');
    self.cleartext._endflush(function() {
      if (self._done !== true) {
        try {
          self._ssl.shutdown();
          self._cycle();
...
SecurePair.prototype._cycle = function() {
  ...
  while (this._encInPending.length > 0 && this._done === false) {
  ...
  while (this._clearInPending.length > 0 && this._done === false) {
  ...
  function mover(reader, writer, checker) {
    ...
    do {
      ...
      } while (self._done === false && (chunkBytes > 0) && (pool.used + bytesRead < pool.length));

      if (bytesRead > 0) {
        chunk = pool.slice(0, bytesRead);
        writer(chunk);
      }
    } while (self._done === false && checker(bytesRead));
  }
...
SecurePair.prototype.destroy = function(err)
{
  this._destroy();
}
...
SecurePair.prototype._destroy = function(err)
{
  ...
  if (!this._done) {
    ...
    if (this.credentials.context) {
      this.credentials.context.close();
      this.credentials.context =  null;
    }
    this.credentials = null;
    process.nextTick(function() {
      self.emit('end', err);
      self.cleartext.emit('end', err);
      self.emit('close', err);
      self.cleartext.emit('close', err);
    });
  }
};
...
SecurePair.prototype.verifyPeer = function() {
  return this._ssl.verifyPeer(this.credentials.context);
};
